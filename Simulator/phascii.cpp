...
typedef struct {
	...
	int		lineno;
	int		ungetch;				// -1 means not set, else a char
	int		eof;					// used by the read call back rcb
	int		ungettoken;
	int		token;
...
/**********************************************************************
 * Insert an element after 'curr'.
 * The new node is returned.
 *
 * This function requires that 'curr' be non-null.
 *
 * RETURNS:
 *      The new element is returned on success,
 *      or NULL on error.
 */
static DLIST *dlist_insert_after(DLIST *curr, void *p)
{
	DLIST *newp;
...
static void error(const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	vsnprintf(cf_errorbuf, BUFSIZ, fmt, ap);
	va_end(ap);

	longjmp(jmpbuf, 1);
}

static void cferror(CONFIG_FILE* cf, const char *fmt, ...)
{
	va_list ap;

	va_start(ap, fmt);
	vsnprintf(cf->error, BUFSIZ, fmt, ap);
	va_end(ap);
}

static void errfmt(const char *fmt, ...)
{
	va_list ap;
 ...
 *	-1 out of memory.
 */
int expand_strings(const char *string, DLIST **result)
{
	char *p, *q;
	char head[ BUFSIZ ];
	char tail[ BUFSIZ ];
		...
		for(curr=expansions; curr; curr=curr->next) {
			...
			*q = '\0';
			for(curr2=ss; curr2; curr2=curr2->next) {
				snprintf(buf, BUFSIZ, "%s%s%s", head, (char*)curr2->p, tail);
				p = strdup(buf);
				if( p == NULL )
					return -1;
				dlist_add(&newp, p);
...
static void UnGetChar(int ch, CONFIG_FILE *cf)
{
	if( cf->fp != NULL ) {
		ungetc(ch, cf->fp);
	} else {
...
static int GetChar(CONFIG_FILE *cf)
{
	char ch;
	intptr_t x;
...
#define KFORTH_COMPILE_FAST
	...
	if( cf->fp != NULL ) {
		...
		if( cf->ungetch != -1 ) {
			...
			if( BI < 0 || BI >= BLEN ) {
				BLEN = cf->rcb(Buffer, sizeof(Buffer));
				if( BLEN == 0 ) {
					cf->eof = 1;
					return -1;
				} else {
...
{
	int t;
	DEFINITION *def = NULL, *d, *save_def;
	DLIST *clist;
...
#define GET_COUNT(p) ((int)((long)(p)))
#define COUNT(count) ((void*)((long)(count)))
	...
	case DEF_ARRAY:
		...
		if( t == T_INTEGER ) {
			if( cp == NULL ) {
				...
				cp = dlist_add(count_head, COUNT(count));
			} else
				count = GET_COUNT(cp->p);
...
#else
				if( head == NULL ) {
					curr = dlist_add(&head, i);
				} else {
					curr = dlist_insert_after(curr, i);
				}
#endif
			}
			dlist_destroy(&chead, NULL);
			count = dlist_count(head);
			dlist_add(count_head, COUNT(count));
			inst->n = count;
			ia = (INSTANCE*)malloc( count * sizeof(INSTANCE) );
			inst->u.vec = ia;
 ...
 *	']'
 */
static int expr_gettoken(const char **string)
{
    const char *p;
    char *q;
	int c, i;
...
{
	const char *p;
	DEFINITION *def, *d=NULL;
	INSTANCE *inst, *i=NULL;
	int array_count, idx=0, got_array, t, type, found;
	DLIST *curr;
	INSTANCE *curr2;
...
static int read_a_line(PhasciiReadCB rcb, char *buf, int len)
{
	char ch;
	char *p;
	intptr_t x;
...
static PHASCII_FILE Do_Open_For_Write(const char *filename, PhasciiWriteCB wcb)
{
	CONFIG_FILE *cf;
	FILE *fp;
	...
	if( wcb == NULL ) {
		fp = fopen(filename, "w");
		if( fp == NULL ) {
			errfmt("%s: %s", filename, strerror(errno));
			return NULL;
		}
	}
	else
...
static PHASCII_FILE Do_Open_For_Read(const char *filename, PhasciiReadCB rcb)
{
	CONFIG_FILE *cf;
	FILE *fp;
	char buf[ BUFSIZ ];
	...
	{
		read_a_line(rcb, buf, sizeof(buf));
		fp = NULL;
	}
	...
	if( n == FALSE ) {
		...
		errfmt("%s: Line 1, PHOTON ASCII Header missing.", filename);
		return NULL;
	}
...
/***********************************************************************
 * Open a config filename. 'mode' can be "r" or "w"
 *
 * RETURNS:
 *	NULL - failure.
 *	else a config file handle.
 */
static PHASCII_FILE Do_Open(const char *filename,
			const char *mode, PhasciiReadCB rcb, PhasciiWriteCB wcb)
{
	if( mode[0] == 'r' ) {
		return Do_Open_For_Read(filename, rcb);
	} else {
		ASSERT( mode[0] == 'w' );
		return Do_Open_For_Write(filename, wcb);
	}
}
 ...
 */
int Phascii_IsInstance(PHASCII_INSTANCE instance, const char *name)
{
	CONFIG_INSTANCE *ci;
...
PHASCII_INSTANCE Phascii_GetInstance(PHASCII_FILE phf)
{
	 ...
	 */
	if( setjmp(jmpbuf) ) {
		snprintf(buf, BUFSIZ, "%s: Line: %d, %s.", cf->filename, cf->lineno, cf_errorbuf);
		strcpy(cf->error, buf);
		return NULL;
	}
 ...
 *
 */
int Phascii_Get(PHASCII_INSTANCE ph_entry, const char *fmt, ...)
{
	CONFIG_INSTANCE *ci;
	CONFIG_FILE *cf;
	va_list ap;
	...
	if( setjmp(jmpbuf) ) {
		...
		if( cf_errorbuf[0] == '*' ) {
			 ...
			 */
			cferror(cf, "*Phascii_Get(): %s", cf_errorbuf);
		} else
			cferror(cf, "Phascii_Get(): %s", cf_errorbuf);
		return num_gotten;
	}
 ...
 */
char *Phascii_MakeString(const char *buf)
{
	char *q, *newp;
    const char *p;
	int count;
 ...
 *
 */
PHASCII_SCHEMA *Phascii_GetSchema(PHASCII_INSTANCE ph_entry, const char *expr)
{
	CONFIG_FILE *cf;
	CONFIG_INSTANCE *ci;
	const char *p;
	DLIST *curr;
	DEFINITION *def, *d=NULL, *next;
	...
	t = expr_gettoken(&p);
	if( t != ET_IDENT ) {
		cferror(cf, "syntax error");
		return NULL;
	}

	if( ident_cmp(etoken_ident, def->name) ) {
		cferror(cf, "identifier %s does not match instance name",
								etoken_ident);
		return NULL;
	}
	...
	for(;;) {
		...
		switch( def->type ) {
			case DEF_STRUCT:
				...
				if( !found ) {
					cferror(cf, "no such component %s.%s",
						def->name, etoken_ident);
					return NULL;
				}
...
//////////////////////////////////////////////////////////////////////
//
// This version opens a file, "r" or "w" work.
//
PHASCII_FILE Phascii_Open(const char *filename, const char *mode)
{
	return Do_Open(filename, mode, NULL, NULL);
}

PHASCII_FILE Phascii_Open_ReadCB(const char *name, PhasciiReadCB rcb)
{
	return Do_Open(name, "r", rcb, NULL);
}

PHASCII_FILE Phascii_Open_WriteCB(const char *name, PhasciiWriteCB wcb)
{
	return Do_Open(name, "w", NULL, wcb);
}

void Phascii_Printf(PHASCII_FILE phf, const char *fmt, ...)
{
	static char buf[5000];		// STATIC BUFFER 5K buffer to hold any printf data
